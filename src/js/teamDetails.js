...
export default class Team {
    ...
    async fetchTeams(league, season) {
        const cachedData = getCachedData(`${this.CACHE_KEY_TEAMS}_${league}_${season}`);

        if (cachedData) {
            console.log("Loaded from cache:", `cache_${this.CACHE_KEY_TEAMS}_${league}_${season}`);
            return cachedData;
        }
        ...
        try {
            ...
            setCachedData(`${this.CACHE_KEY_TEAMS}_${league}_${season}`, result.response);
    ...
    async renderTeams(league, season, scroller) {
        ...
        if (!scroller) {
            ...
            console.log("Team-Items lengh:", teamsHTMLDuplicated.length)
    ...
    async fetchTeamInfo(teamId) {
        ...
        try {
            ...
            const result = await this.footballAPI.fetchFootballData(endpoint);
            setCachedData(`${this.CACHE_KEY_TEAM_INFO}_${teamId}`, result.response[0]);
